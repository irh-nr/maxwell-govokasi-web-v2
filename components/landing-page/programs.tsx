...
export default function Programs() {
  return (
    <section id="programs" className="relative">
      ...
      />
      <Image src={"/photos/3067.jpg"} alt="" fill className="object-cover" />
      {/* VIGNETTE OVERLAY: Darker on the left for text, lighter on the right */}
      <div className="absolute inset-0 bg-slate-900/30 " />

      <div className="relative z-10 py-24 md:px-12 bg-linear-to-b from-white/5 to-white backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col lg:flex-row gap-x-24 gap-y-10">
            {/* Left Sidebar: Title */}
            <div className="min-w-[120px] mt-2">
              <div className="sticky top-32">
                <h2 className="text-[14px] font-bold uppercase tracking-[0.2em] leading-tight text-white drop-shadow-lg">
                  Program <br /> Pathways
                </h2>
                <div className="mt-3 h-1 w-16 bg-gradient-to-r from-white to-transparent rounded-full" />
              </div>
            </div>

            {/* Right Content */}
            <div className="lg:w-3/4 space-y-16">
              <h1 className="text-[30px] font-bold leading-tight max-w-2xl text-balance text-white drop-shadow-lg">
              ...
              <div className="grid grid-cols-1 gap-12">
                {PROGRAM_DATA.map((program) => (
                  <div
                    key={program.id}
                    className="group p-8 md:p-10 bg-white/90 backdrop-blur-md rounded-[24px] border border-border/30 hover:border-primary/40 shadow-sm transition-all duration-300 card-depth-v2"
                  >
                    {/* Icon & Title */}
